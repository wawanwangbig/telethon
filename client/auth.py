...
from ..errors import InvalidCodeError, ReceivePhoneCodeError
...
class AuthMethods:
    ...
    async def _start(
        ...
        while attempts < max_attempts:
            try:
                ...
                break
            except errors.PhoneCodeEmptyError as e:
                reason = e.args[0]
            except errors.PhoneCodeExpiredError as e:
                reason = e.args[0]
            except errors.PhoneCodeHashEmptyError as e:
                reason = e.args[0]
            except errors.PhoneCodeInvalidError as e:
                reason = e.args[0]
            except ReceivePhoneCodeError as e:
                reason = e.args[0]

            attempts += 1
        else:
            raise InvalidCodeError(
                '{} consecutive sign-in attempts failed. Reason: {}'
                .format(max_attempts, reason)
            )
        ...
        try:
            logging.info(signed + name + tos)
        except UnicodeEncodeError:
            # Some terminals don't support certain characters
            logging.error(signed + name.encode('utf-8', errors='ignore').decode('ascii', errors='ignore') + tos)
    ...
    async def sign_up(
        ...
        """
        raise ValueError(
            'Third-party applications cannot sign up for Telegram. See https://github.com/LonamiWebs/Telethon/issues/4050 for details')
